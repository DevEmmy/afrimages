...
const Layout = ({
  ...
  return (
    <div className="flex flex-col gap-10">
      <div className=" px-[5%] py-2 flex flex-col gap-2 border border-b fixed top-0 left-0 right-0 z-[100] bg-white">
        <nav className="flex-between">
          ...
          <div className="lg:hidden">
            <RiMenu2Line
              className="text-green700 text-2xl cursor-pointer font-bold"
              onClick={() => setIsOpen(true)}
            />
          </div>
          <Menu isOpen={isOpen} setIsOpen={setIsOpen} color="text-green-700" />
          <div className="items-center gap-10 hidden lg:flex">
            <Link href={"/"} className="text-green700 text-[32px] font-bold">
              Afrimages
            </Link>
          ...
          <Link
            href={"/login"}
            className="border-2 border-green700 text-green700 px-5 py-2 rounded-md"
          >
            Login
          </Link>
        </nav>
      ...
      <div className="mx-[5%] mt-40">{children}</div>
